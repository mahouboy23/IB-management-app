...
const AssignStudent = async (studentId) => {
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <>
          ...
          <button onClick={() => handleDelete(row.original.class_id)}>Delete</button>
          <button onClick={() => handleAssignStudent(row.original)}>Assign Student</button>
        </>
      )
    }
  ], []);
  ...
  return (
    <div>
      ...
      <button onClick={() => setModalIsOpen(true)}>Add New Class</button>
      <Modal
        isOpen={modalIsOpen}
  ...
  >
    ...
    {teachers.map(teacher => (
      ...
      <Modal isOpen={assignModalIsOpen} onRequestClose={() => setAssignModalIsOpen(false)} contentLabel="Assign Student">
        <h2>Assign Student to Class</h2>
        <input type="text" placeholder="Search students" value={searchStudent} onChange={(e) => setSearchStudent(e.target.value)} />
        <ul>
            {availableStudents.filter(student => student.full_name.toLowerCase().includes(searchStudent.toLowerCase())).map(student => (
    ...
    return (
        <>
            <tr {...row.getRowProps()}>
                ...
                <td>
                    <button onClick={() => toggleExpandedClass(row.original.class_id)}>
                    ...
                    <td colSpan={columns.length + 1}>
                        ...
                        <ul>
                            {expandedClass.students.map(student => (
                                <li key={student.user_id}>
                                    {student.full_name}
                                    <button onClick={() => removeStudent(row.original.class_id, student.user_id)}>
                                        Remove
                                    </button>
                                </li>
                            ))}
